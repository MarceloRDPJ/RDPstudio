(()=>{
  ...
  const state={cart:[],scenario:'discovery',selectedProduct:null};
  ...
  function message(who,text,meta=''){
    ...
    $('#sales-messages').append(node);
    while($('#sales-messages').children.length>14)$('#sales-messages').firstElementChild.remove();
    $('#sales-messages').scrollTop=$('#sales-messages').scrollHeight;
  }
  ...
  function addToCart(product,quantity=1){
    ...
    trace('update_cart',`${product.id} · quantidade ${safe}`);renderCart();message('assistant',`${safe} ${safe===1?'unidade adicionada':'unidades adicionadas'} ao carrinho local. Nenhuma reserva real foi feita.`);suggestions(['Revisar pedido','Ver outro produto']);
  }
  async function runScenario(key){
    ...
    if(scenario.product){const product=catalog.find(item=>item.id===scenario.product);state.selectedProduct=product;trace('get_product',`${product.id} · estoque ${product.stock}`);productCard(product)}
  ...
  async function handle(raw){
    ...
    if(/ver outro|outro produto|mostrar produtos/.test(normalized)){trace('parse_intent','list_catalog');catalog.forEach(productCard);message('assistant','Estes são os três itens disponíveis no catálogo demonstrativo. Você pode perguntar por qualquer um deles.');suggestions(['Tem máscara para cílios?','Quero ver o gloss']);$('#trace-state').textContent='Concluído';return}
    if(/adicionar|carrinho/.test(normalized)&&(product||state.selectedProduct)){const selected=product||state.selectedProduct;trace('parse_intent','update_cart');trace('get_product',`${selected.id} · estoque ${selected.stock}`);addToCart(selected,quantity);$('#trace-state').textContent='Concluído';return}
